...
validate.validators.checkUsername = function (value,options) {
    if (value != null) {
        return validate_func_check_user_name(value);
    }
};

function validate_func_check_user_name(username) {
        ...
        if (username.length >4)
        {
            if(username.length <16)
            {
                if (/^[a-z0-9_.]+$/.test(username))
                {
                    ...
                    {
                        return 'نام کاربری نمیتواند شامل __ باشد';
                    }
                }
                else
                {
                    return 'نام کاربری فقط میتواند شامل حروف انگلیسی کوچک ، اعداد ، نقطه و خط تیره باشد';
                }

            }
            else
            {
                return 'نام کاربری باید کمتر از 20 حرف باشد';
            }
        }
        else
        {
            return 'نام کاربری باید بزرگتر از 5 حرف باشد';
        }
    }
    else
    {
        console.log('f');
        return 'نام کاربری باید با حروف کوچک انگلیسی شروع گردد.' ;
    }
}
